...
#define NUM_PAGES       128     /* Number of memory pages. The pages are numbered 1..NUM_PAGES */
...
/*
 * Memory allocation on the basis of Recent Usage.
 * PageFrames = The current table
 * num_frames = how many frames in you table
 * PageAccesses = id of pages in the order of the access, index is the time of access i.e., PageAccesses[i]=is the id of the page accessed at time i
 * num_accesses = How many access is in the PageAccesses
...
/*
 * Memory allocation on the basis of Future Access, NOT a general purpose solution.
 * PageFrames = The current table
 * num_frames = how many frames in you table
 * PageAccesses = id of pages in the order of the access, index is the time of access i.e., PageAccesses[i]=is the id of the page accessed at time i
 * num_accesses = How many access is in the PageAccesses
...
int PRAlgo_OPT(const PageFrame * PageFrames, int num_frames, const int * PageAccesses, int num_accesses, int current_access)
{
  ...
  int futurepage_access_time[num_frames];
  for(int i=0; i<num_frames; i++) {
    futurepage_access_time[i] = num_accesses;
  }

  for(int i=0; i < num_frames; i++) {
    for(int j=current_access; j < num_accesses; j++) {
      if((PageAccesses[j] == PageFrames[i].page_id)) {
        futurepage_access_time[i] = j;
        break;
      }
    }
  }
  ...
  for(int i=1; i<num_frames; i++) {
    if(futurepage_access_time[i] == num_accesses) {
      return i;
    }
    else if(futurepage_access_time[i] > futurepage_access_time[max]) {
      max = i;
    }
  }
  return max;
...
/*
 * Memory allocation on the basis of custom algorithm.
 * PageFrames = The current table
 * num_frames = how many frames in you table
 * PageAccesses = id of pages in the order of the access, index is the time of access i.e., PageAccesses[i]=is the id of the page accessed at time i
 * num_accesses = How many access is in the PageAccesses
...
int PRAlgo_CUST(const PageFrame * PageFrames, int num_frames, const int * PageAccesses, int num_accesses, int current_access)
{
  ...
  int max = 0;
  for(int i=1; i<num_frames; i++) {
    if(futurepage_access_time[i] == 0) {
      return i;
    }
    if(futurepage_access_time[i] > futurepage_access_time[max]) {
      max = i;
    }
  }
  return max;
